...
def is_missing_supplier_gst_error(error):
    """Detect DB errors caused by a missing suppliers.gst column."""
...
def get_supplier_gst(data):
    """Accept GST from current and legacy request keys."""
    gst_value = data.get('gst')
    if gst_value is None:
        gst_value = data.get('gst_number')
    if gst_value is None:
        gst_value = data.get('gstNumber')
...
def update_supplier(supplier_id):
    ...
    try:
        ...
        if 'gst' in data or 'gst_number' in data or 'gstNumber' in data:
            supplier.gst = get_supplier_gst(data)
        ...
        try:
            ...
            db.session.rollback()
            if not is_missing_supplier_gst_error(update_error):
                raise
